...
async function generateBwrapLauncher(config: RcktConfig, appDir: string): Promise<string> {
  ...
  // Pick main binary: use config.name, fallback to first alphabetically
  let mainBin = config.name
  try {
    const bins = (await fs.readdir(path.join(appDir, 'bin'))).filter(f => !f.startsWith('.'))
    if (bins.length > 0) mainBin = bins.includes(config.name) ? config.name : bins.sort()[0]
  } catch { /* use config.name */ }

  const args: string[] = [
    // ── Core filesystem ───────────────────────────────────────────────────────
    `  --ro-bind /usr /usr`,
    // Symlinks for /bin → /usr/bin etc. (modern distros have /bin as symlink)
    `  --symlink usr/bin    /bin`,
    `  --symlink usr/bin    /sbin`,
    `  --symlink usr/lib    /lib`,
    `  --symlink usr/lib64  /lib64`,
    // Fallback for distros where /lib is NOT a symlink (Fedora, some Arch setups)
  ...
  // ── Home directory ────────────────────────────────────────────────────────
  if (perms.home === 'readonly')
    args.push(`  --ro-bind "$HOME" "$HOME"`)
  else if (perms.home === 'readwrite')
    args.push(`  --bind "$HOME" "$HOME"`)
  else
    // home: none — tmpfs so apps don't crash looking for $HOME
  ...
  if (perms.display !== false) {
    ...
    args.push(`  --ro-bind-try "\${XDG_RUNTIME_DIR:-/run/user/$(id -u)}" "\${XDG_RUNTIME_DIR:-/run/user/$(id -u)}"`)
    args.push(`  --setenv XDG_RUNTIME_DIR "\${XDG_RUNTIME_DIR:-/run/user/$(id -u)}"`)
    args.push(`  --setenv WAYLAND_DISPLAY "\${WAYLAND_DISPLAY:-wayland-0}"`)
    args.push(`  --setenv DBUS_SESSION_BUS_ADDRESS "\${DBUS_SESSION_BUS_ADDRESS:-}"`)
  }

  // ── Audio (PulseAudio/PipeWire) ───────────────────────────────────────────
  if (perms.audio === true) {
    args.push(`  --ro-bind-try "/run/user/$(id -u)/pulse"    "/run/user/$(id -u)/pulse"`)
    args.push(`  --ro-bind-try "/run/user/$(id -u)/pipewire-0" "/run/user/$(id -u)/pipewire-0"`)
    args.push(`  --setenv PULSE_SERVER "unix:/run/user/$(id -u)/pulse/native"`)
  }
